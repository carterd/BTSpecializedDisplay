#include "MotorPowerSmall.h"

#include <LvglThemes/lv_theme_binary.h>


MotorPowerSmall::MotorPowerSmall() {
}

lv_obj_t* MotorPowerSmall::createLvObj(lv_obj_t* parent) {    
    ...
    lv_label_set_text(label_obj, "eWatts:");
...
void MotorPowerSmall::focusLvObj(BaseLvObject* defocusLvObj)
{    
	// The LVObj that'll get the refreshes and should there hook into updates    
    this->bluetoothBikeController->getConnectedBluetoothBike().readBikeStateAttribute(BikeStateAttributeIndex::MOTOR_POWER, MonitorAttributeType::EVERY_SECOND);
	this->update();
}

void MotorPowerSmall::statusUpdate()
{
    this->update();
}

void MotorPowerSmall::update() {
    char valueString[32];
    uint16_t power = this->bluetoothBikeController->getBikeState().getStateAttribute(BikeStateAttributeIndex::MOTOR_POWER).bikeStateAttributeValue.valueUint16;
